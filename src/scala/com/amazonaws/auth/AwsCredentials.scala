...
object AwsCredentials extends ((String, String, Option[String], Option[Date]) => AwsCredentials) {
  ...
  lazy val fromConfiguration: AwsCredentials = {
    val accessKey = current.configuration.getString("aws.accessKey").get
    val secretKey = current.configuration.getString("aws.secretKey").get
    println(accessKey, secretKey)
    SimpleAwsCredentials(accessKey, secretKey)
    }
